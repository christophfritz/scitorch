...
class TestToBytes(object):
    def test_to_bytes_default_values_scalar(self):
        bytes = to_bytes()
        assert torch.all(torch.eq(bytes, T(0)))
    ...
    def test_to_bytes_with_dimension(self):
        ...
        assert torch.all(torch.eq(bytes['val'], T([0, constants.giga]))) and bytes['dim'] == 'B'
...
class TestToBytesFromByteUnits(object):
    def test_to_bytes_from_byte_scalar(self):
        bytes = to_bytes(1, 'B')
        assert torch.all(torch.eq(bytes, T(1)))
    ...
    def test_to_bytes_from_kilobyte_scalar(self):
        bytes = to_bytes(1, 'KB')
        assert torch.all(torch.eq(bytes, T(constants.kilo)))

    def test_to_bytes_from_kilobyte_list(self):
        bytes = to_bytes([0, 1], 'KB')
        assert torch.all(torch.eq(bytes, T([0, constants.kilo])))

    def test_to_bytes_from_megabyte_scalar(self):
        bytes = to_bytes(1, 'MB')
        assert torch.all(torch.eq(bytes, T(constants.mega)))

    def test_to_bytes_from_megabyte_list(self):
        bytes = to_bytes([0, 1], 'MB')
        assert torch.all(torch.eq(bytes, T([0, constants.mega])))

    def test_to_bytes_from_gigabyte_scalar(self):
        bytes = to_bytes(1, 'GB')
        assert torch.all(torch.eq(bytes, T(constants.giga)))

    def test_to_bytes_from_gigaobyte_list(self):
        bytes = to_bytes([0, 1], 'GB')
        assert torch.all(torch.eq(bytes, T([0, constants.giga])))

    def test_to_bytes_from_terabyte_scalar(self):
        bytes = to_bytes(1, 'TB')
        assert torch.all(torch.eq(bytes, T(constants.tera)))

    def test_to_bytes_from_terabyte_list(self):
        bytes = to_bytes([0, 1], 'TB')
        assert torch.all(torch.eq(bytes, T([0, constants.tera])))

    def test_to_bytes_from_petabyte_scalar(self):
        bytes = to_bytes(1, 'PB')
        assert torch.all(torch.eq(bytes, T(constants.peta)))
