...
from scitorch.conversion.temperature import *
...
class TestToKelvin(object):
    ...
    def test_to_kelvin_with_dimension(self):
        ...
        assert torch.all(torch.eq(kelvin['val'], T([273.15, 0]))) and kelvin['dim'] == 'K'
...
class TestToCelsius(object):
    ...
    def test_to_celsius_with_dimension(self):
        ...
        assert torch.all(torch.eq(celsius['val'], T([0, -15]))) and celsius['dim'] == 'C'
...
class TestToFahrenheit(object):
    ...
    def test_to_fahrenheit_with_dimension(self):
        ...
        assert torch.all(torch.eq(fahrenheit['val'], T([32, 5]))) and fahrenheit['dim'] == 'F'
