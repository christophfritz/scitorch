...
from scitorch.conversion.digital_storage import to_bytes, to_bits
...
class TestToBytesFromByteUnits(object):
    ...
    def test_to_bytes_from_kilobyte_scalar(self):
        bytes = to_bytes(1, 'KB')
        assert torch.all(torch.eq(bytes, _create_tensor(1e3)))

    def test_to_bytes_from_kilobyte_list(self):
        bytes = to_bytes([0, 1], 'KB')
        assert torch.all(torch.eq(bytes, _create_tensor([0, 1e3])))

    def test_to_bytes_from_megabyte_scalar(self):
        bytes = to_bytes(1, 'MB')
        assert torch.all(torch.eq(bytes, _create_tensor(1e6)))

    def test_to_bytes_from_megabyte_list(self):
        bytes = to_bytes([0, 1], 'MB')
        assert torch.all(torch.eq(bytes, _create_tensor([0, 1e6])))

    def test_to_bytes_from_gigabyte_scalar(self):
        bytes = to_bytes(1, 'GB')
        assert torch.all(torch.eq(bytes, _create_tensor(1e9)))

    def test_to_bytes_from_gigaobyte_list(self):
        bytes = to_bytes([0, 1], 'GB')
        assert torch.all(torch.eq(bytes, _create_tensor([0, 1e9])))

    def test_to_bytes_from_terabyte_scalar(self):
        bytes = to_bytes(1, 'TB')
        assert torch.all(torch.eq(bytes, _create_tensor(1e12)))

    def test_to_bytes_from_terabyte_list(self):
        bytes = to_bytes([0, 1], 'TB')
        assert torch.all(torch.eq(bytes, _create_tensor([0, 1e12])))

    def test_to_bytes_from_petabyte_scalar(self):
        bytes = to_bytes(1, 'PB')
        assert torch.all(torch.eq(bytes, _create_tensor(1e15)))

    def test_to_bytes_from_petabyte_list(self):
        bytes = to_bytes([0, 1], 'PB')
        assert torch.all(torch.eq(bytes, _create_tensor([0, 1e15])))
...
#             to_bits('b', [0, 1])
#
#     def test_to_bits_tensor(self):
#         bit_tensor = torch.tensor([0, 1])
#         bits = to_bits(bit_tensor)
#         assert torch.all(torch.eq(bits, _create_tensor([0, 1])))
#
#     def test_to_bits_wrong_scale_scalar(self):
#         with raises(NotImplementedError):
#             to_bits(1, 'kb')
#
#     def test_to_bits_wrong_scale_list(self):
#         with raises(NotImplementedError):
#             to_bits([0, 1], 'kb')